...
class NodoGenerador(Nodo):
    ...
    def genera_arbol(self, env):
        ...
        if self.id_nodo == 0:
            ...
            go_nodo = ("GO", self.id_nodo)
            env.process(self.canal_salida.envia(go_nodo, self.vecinos))

        while True:
            # mensaje_recibido = yield self.canal_entrada.get()
            mensaje = yield self.canal_entrada.get()
            accion, id_recibido = mensaje
            ...
            if accion == "GO":
                if self.padre is None:
                    ...
                    if self.mensajes_esperados == 0:
                        ...
                        back_nodo = ("Back", self.id_nodo)
                        yield env.process(self.canal_salida.envia(back_nodo, [self.padre]))
                    else:
                        # Reenvía "GO" a sus vecinos, excepto el padre
                        go_nodo = ("GO", self.id_nodo)
                        env.process(self.canal_salida.envia(go_nodo, [x for x in self.vecinos if x != self.padre]))
                else:
                    # Si ya tiene padre, responde con "Back" indicando que el nodo remitente no es su padre
                    back_none = ("Back", None)
                    env.process(self.canal_salida.envia(back_none, [id_recibido]))
                ...
                if self.mensajes_esperados == 0 and self.padre is not None:
                    ...
                    if self.id_nodo != 0:
                        back_nodo = ("Back", self.id_nodo)
                        env.process(self.canal_salida.envia( back_nodo, [self.padre]))
